...
extension NetworkManager: AddressRequestable {
  ...
  func fetchTransactionSummaries(for addresses: [String]) -> Promise<[AddressTransactionSummaryResponse]> {
    return atsResponses(for: addresses)
  }
  ...
  private func atsResponses(
    ...
    return cnProvider.requestList(AddressesTarget.query(addresses, page, perPage))
      .get { responses in
      ...
      .then { (responses: [AddressTransactionSummaryResponse]) -> Promise<[AddressTransactionSummaryResponse]> in
        ...
        return self.atsResponses(for: addresses, page: nextPage, perPage: perPage, with: combined)
    }
  }
